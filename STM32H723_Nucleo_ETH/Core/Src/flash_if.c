...
void Jump_To_Application(void)
{
    ...
    printf("\rJump to application set MSP\n");
...
/**
  ...
  *         1: error occurred
  */
uint32_t FLASH_If_Erase(uint32_t StartSector, uint32_t NumSec)
{
...
HAL_StatusTypeDef FLASH_If_Copy(uint32_t dest_address, uint32_t src_address, uint32_t size)
{
    HAL_StatusTypeDef status = HAL_OK;
    uint32_t flash_data[8]; // Buffer to hold 32 bytes (8 words)

    HAL_FLASH_Unlock();
    SCB_DisableICache();
    for (uint32_t i = 0; (i < size) && (dest_address <= (USER_FLASH_END_ADDRESS - 32)); i += 32)
    {
        // Copy 32 bytes from src_address to buffer
        memcpy(flash_data, (void*)(src_address + i), 32);

        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, dest_address, (uint32_t)flash_data) == HAL_OK)
        {
            // Move to the next 32-byte block
            dest_address += 32;
        }
        else
        {
...
uint16_t FLASH_If_GetWriteProtectionStatus(void)
{
  ...
  if (APPLICATION_ADDRESS < USER_FLASH_END_ADDRESS)
  {
    /* Select Bank1 */
